...
if current_tab == "👑 칭찬 승인/확인":
    ...
    if user_role == "admin":
        ...
        # 3. Reward Logs Editor
        # Fetch ALL logs to manage updates correctly
        try:
            ...
            # Note: Logs use Name, so we match target_child_name
            ...
            if reward_logs_view.empty:
                ...
                edited_rewards = st.data_editor(
                    ...
                    hide_index=True,
                    width="stretch",
                    key="praise_reward_editor",
                    num_rows="dynamic" # Allow Deletion
                )
                
                if st.button("💾 보상 이력 저장 (Save Rewards)", key="save_reward_logs"):
                    def save_rewards_action():
                        ...
                        # 2. Get Edited 
                        df_subset_new = edited_rewards.copy()
                        if "__id" in df_subset_new.columns:
                                del df_subset_new["__id"]
                        if "__id" in df_others.columns:
                                del df_others["__id"]
                                
                        # Re-approach for Merge:
                        # Map edits back to original rows via __id
                        original_ids = reward_logs_view['__id'].tolist()
                        current_ids = edited_rewards['__id'].dropna().astype(int).tolist() # IDs that survived
                        
                        ids_to_delete = set(original_ids) - set(current_ids)
                        
                        # Apply Deletions to main
